          ...
          <form id='contact-form' onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="grey-text">
              <MDBInput name='ClientName' label="Your name" icon="user" group type="text" validate error="wrong"
                success="right" {...register('ClientName', { required: true })}
              />
              {errors.ClientName?.type === 'required' && "Your  full name is required"}
              <MDBInput name='emaill' label="Your email" icon="envelope" group type="email" validate error="wrong"
                success="right" {...register('emaill', { required: true })}
              />
              {errors.email?.type === 'required' && "Your  email is required"}
              {/* <div className='form-select'>
                <i data-test="fa" className="fa fa-tag prefix md-form icon-forms"></i>
                <select className="browser-default custom-select  select-customize"  {...register("events")} name='eventss'>
              ...
              <MDBRow>
                <MDBCol md='6'>
                  <MDBInput name='phoneThrough' label="Phone number" icon="phone-alt" group type="tel"
                    validate error="wrong" success="right"
                    {...register('phoneThrough', { required: true })}
                  />
                  {errors.phoneThrough?.type === 'required' && "Your  phone number is required"}
                </MDBCol>
                <MDBCol md='6'>
                  <MDBInput name='heard' label="How did you hear about us" icon="users" group type="text" validate error="wrong"
                    success="right" {...register('heard', { required: false })}
                  />
                </MDBCol>
              </MDBRow>
              <MDBInput name='dayDate' label="Wedding date" icon="calendar" group type="text"
